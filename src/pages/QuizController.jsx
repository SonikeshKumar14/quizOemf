...
const QuizController = (CUId) => {
    ...
    const getExams = async () => {
        const { data } = await axios.get(`${apiURL}/examquestions/` + id.id);
        setQuestions(data);
        userCheck();
    }
    ...
    const securityData = async () => {
        axios.all([
            await axios.get(`${apiURL}/users/` + CUId.CUId),
            await axios.get(`${apiURL}/exam/exam/` + id.id)
            ...
            if (data2.data[0].creatorUserId == CUId.CUId) {
                ...
                axios.post(`${apiURL}/userexams/`, dummyData).then((response) => {
    ...
    const userCheck = async () => {
        try {
            const { data } = await axios.get(`${apiURL}/userexams/` + CUId.CUId);
