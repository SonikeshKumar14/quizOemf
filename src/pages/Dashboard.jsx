...
const Dashboard = (CUId) => {
  ...
  const URLF = process.env.REACT_APP_URLF
  ...
  const getExamNames = async () => {
    const { data } = await axios.get(`${apiURL}/exam/${CUId.CUId}`);
    setExamNameStorage(data);
    setIsLoading(false);
  }

  const deleteExam = (id) => {
    axios.delete(`${apiURL}/exam/${id}`).then((response) => {
  ...
  const handleName = (e) => {
    ...
    if (examName == "") {
      ...
      console.log(newExam)
      axios.post(`${apiURL}/exam/`, newExam).then((response) => {
        console.log(response.status);
        console.log(response.data);
      });
      setDummy(dummy + 1)
            ...
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
              ...
              <TableBody>
                {examNameStorage.map((name) => (
                  ...
                  >
                    <TableCell component="th" scope="row" onClick={() => { navigator.clipboard.writeText(`${URLF}/quiz/` + name._id) }}>
