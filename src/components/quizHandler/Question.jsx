...
const Question = ({
  ...
  const handleCreatorUser = async () => {
    const { data } = await axios.get(`${apiURL}/exam/exam/` + id.id)
  ...
  const handleSubmit = (e) => {
    ...
    if (pass == userId) {
      ...
      axios.patch(`${apiURL}/userexams/${exam_id}`, userExam).then((response) => {
  ...
  const handleReview = (i) => {
    if (pass == userId) {
      ...
      axios.put(`${apiURL}/userexams/` + exam_id, userOptions).then((response) => {
        console.log(response.status);
        console.log(response.data);
      });
    }
